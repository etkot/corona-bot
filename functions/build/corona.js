...
const functions = require("firebase-functions"); // Telegram api url for the bot
...
exports.sendCorona = async chat_id => {
  try {
    ...
    sickPeople.forEach(person => {
      CasesByPlace[person.healthCareDistrict] = CasesByPlace[person.healthCareDistrict] ? CasesByPlace[person.healthCareDistrict] + 1 : 1;
    });
    recoveredPeople.forEach(person => {
      RecoveredByPlace[person.healthCareDistrict] = RecoveredByPlace[person.healthCareDistrict] ? RecoveredByPlace[person.healthCareDistrict] + 1 : 1;
    });
    deadPeople.forEach(person => {
      DeathsByPlace[person.healthCareDistrict] = DeathsByPlace[person.healthCareDistrict] ? DeathsByPlace[person.healthCareDistrict] + 1 : 1;
    });
